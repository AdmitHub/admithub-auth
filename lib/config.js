...
if (Meteor.isServer) {
  Meteor.startup(function() {
    ...
    Accounts.emailTemplates.from = "AdmitHub <bot@admithub.com>";
  });
}
...
AccountsTemplates.addField({
  _id: 'subscribedToNewsletter',
  type: 'checkbox',
  displayName: "Sign up for our admissions newsletter"
});
...
if (Meteor.isServer) {
  ...
  Accounts.onCreateUser(function(options, user) {
    if (options.profile) {
      ...
      if (options.profile.utm_medium) {
        var utm_medium = options.profile.utm_medium;
        delete options.profile.utm_medium;
      }
    }

    var firstName = dotGet(user, "services.facebook.first_name");
    var lastName = dotGet(user, "services.facebook.last_name");

    CollegeProfiles.insert({
      userId: user._id,
      created: new Date(),
      firstName: firstName,
      lastName: lastName,
